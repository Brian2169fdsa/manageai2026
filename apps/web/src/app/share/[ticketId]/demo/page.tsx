...
async function getArtifactHtml(ticketId: string, type: 'solution_demo' | 'build_plan'): Promise<string | null> {
...
export default async function ShareDemoPage({
  ...
  const [html, ticket] = await Promise.all([
    getArtifactHtml(ticketId, 'solution_demo'),
    getTicket(ticketId),
  ]);
        ...
        <body style={{ fontFamily: 'system-ui, sans-serif', background: '#F8F9FB', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', margin: 0 }}>
          <div style={{ textAlign: 'center', maxWidth: 480, padding: '40px 24px' }}>
            ...
            <p style={{ fontSize: 14, color: '#8890A0', lineHeight: 1.6, marginBottom: 24 }}>
              This demo link is either invalid or the solution demo hasn&apos;t been generated yet.
            </p>
            <a
  ...
  // Inject a canonical title if ticket info is available
  const title = ticket
    ? `${ticket.project_name || 'Solution Demo'} — ${ticket.company_name} | ManageAI`
    : 'Solution Demo | ManageAI';

  // We render the full HTML document inside an iframe to avoid React conflicts
  // The iframe gets the full HTML as srcdoc
  return (
    <html lang="en">
      <head>
        <meta charSet="UTF-8" />
